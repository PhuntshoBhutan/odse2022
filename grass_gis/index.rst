************
Introduction
************

Software used in this training materials
========================================

.. figure:: ./images/grass-gis-logo.png
   :class: small
          
* `GRASS GIS <http://grass.osgeo.org>`__ **version 8** for geospatial
  raster, imagery, and vector data processing and providing spatial
  analysis.
* `sentinelsat library <https://pypi.python.org/pypi/sentinelsat>`__
  utility to search and download Copernicus Sentinel satellite images.

Sample data
===========

For purpose of this training materials a `sample data 
<https://geo.fsv.cvut.cz/geoharmonizer/odse_workshop_2022/grass-gis-data.zip>`__ (605MB)
has been created based on Open Data covering ODSE pilot tiles.
          
Content
=======

GRASS GIS 8 (Part 1): Introduction and new features
---------------------------------------------------
              
.. toctree::
   :maxdepth: 2

   units/01
   units/02
   units/03
   units/04
   units/05

GRASS GIS 8 (Part 2): Processing multitemporal EO data
------------------------------------------------------

.. toctree::
   :maxdepth: 2

   units/06
   units/07
   units/08

Lecturer
========

* Martin Landa, GeoForAll Lab, Czech Technical University in Prague,
  Czech Republic
* Markus Neteler, mundialis, Germany
* Ondřej Pešek, GeoForAll Lab, Czech Technical University in Prague,
  Czech Republic

Thanks to Veronica Andreo (Instituto de Altos Estudios Espaciales
"Mario Gulich" - CONICET) for the review.

License
=======

`Creative Commons Attribution-ShareAlike 4.0 International License
<http://creativecommons.org/licenses/by-sa/4.0/>`_.

.. figure:: ./images/cc-by-sa.png 
   :width: 130px
   :scale-latex: 120

Source code available on `GitLab
<https://gitlab.com/geoharmonizer_inea/odse-workshop-2022/-/tree/main/grass_gis>`__. Feel
free to open issues or pull requests to improve the materials :-)

*Document version:* |release| (built |today|)
